...
### Manual release packaging (maintainers)
Release installer expects asset name format: `vnstream_<os>_<arch>.tar.gz` and archive containing `vnstream` binary at root.

```bash
GOOS=linux GOARCH=amd64 go build -o vnstream . && tar -czf vnstream_linux_amd64.tar.gz vnstream
```
...
### Đóng gói release thủ công (cho maintainer)
Script cài đặt yêu cầu asset theo định dạng: `vnstream_<os>_<arch>.tar.gz` và bên trong có file `vnstream` ở root.
